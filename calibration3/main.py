...
def print_schedule(schedule: DeviceSchedule):
    ...
    for slot in schedule.timeslot_list:
        ...
        for tech in technician_list:
            ...
            if len(tech_tasks) == 0:
                ...
                out += "{:<10}".format(reduce(lambda a, b: a + "," + b,
                                              map(lambda t: t.required_skill, tech_tasks)))[0:10] + " | "
        print(out)
        out = "|            | "
        for tech in technician_list:
            tasks = slot_task_map.get(slot, [])
            ...
            if len(tech_tasks) == 0:
                ...
                out += "{:<10}".format(reduce(lambda a, b: a + "," + b,
                                              map(lambda t: t.serial_number, tech_tasks)))[0:10] + " | "
        print(out)
        print("|" + ("------------|" * (len(technician_list) + 1)))
    ...
    if len(unassigned_tasks) > 0:
        ...
        for task in unassigned_tasks:
            ...
            print(f" {task.name} - {technician_name} - {workstation_name} - {task.type} - {task.required_skill} - {task.serial_number}")
